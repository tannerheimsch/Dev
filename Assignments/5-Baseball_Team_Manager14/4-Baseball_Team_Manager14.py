...
def display_lineup(players):
    if not players:
        ...
        for i, player in enumerate(players, start=1):
            player_name = player.fullName  # Use the fullName property of Player
            position = player.position
            at_bats = player.atBats
            hits = player.hits
            avg = player.battingAvg  # Use the battingAvg property of Player
...
def delete_player(players):
    ...
    print(f"{deleted_player.fullName} has been removed from the lineup.\n")
...
def edit_player_position(players):
    ...
    players[position - 1].position = new_position  # Update the position attribute
    print(f"{players[position - 1].fullName}'s role was updated to {new_position}.")
...
def edit_player_stats(players):
    ...
    current_player = players[position - 1]
    print(f"\nCurrent Stats for {current_player.fullName}")
    print(f"Current At Bats: {current_player.atBats}")
    print(f"Current Hits: {current_player.hits}\n")

    at_bats = get_at_bats()
    hits = get_hits(at_bats)
    
    current_player.atBats = at_bats  # Update the atBats attribute
    current_player.hits = hits  # Update the hits attribute
    
    print(f"\n{current_player.fullName}'s stats were updated.")
